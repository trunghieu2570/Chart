...
public class StatisticLineChart extends AppCompatActivity {
    ...
    private void Statistic()
    {
        ...
        for(int i=0;i<fields.length;i++)
        {
            ...
            String str="";
            if(data[data.length-1]>data[0]) str="+ Từ "+unitXName.toLowerCase()+" "+timeLines[0]+" đến "+unitXName.toLowerCase()+" "+timeLines[timeLines.length-1]+": "+valueName.toLowerCase()+" tăng "+df.format(((data[data.length-1]/data[0])-1)*100)+"%";
            else str="+ Từ "+unitXName.toLowerCase()+" "+timeLines[0]+" đến "+unitXName.toLowerCase()+" "+timeLines[timeLines.length-1]+": "+valueName.toLowerCase()+" giảm "+df.format(((1-data[data.length-1]/data[0]))*100)+"%";
        ...
        txt_Name_Line.setText("Nhận xét "+name);

        txt_TuongQuan_Line.setText("Tương quan giữa các "+fieldName.toLowerCase()+":");
        txt_AverageSpeed_Line.setText("Tốc độ tăng trưởng trung bình: "+df.format(getAverageSpeed(values))+" "+unitYName+"/"+unitXName+".");
        txt_MaxSpeed_Line.setText(fieldName+" có tốc độ tăng trưởng nhanh nhất là "+fields[getMaxSpeedPosition(values)]+": "+df.format(maxSpeed)+" "+unitYName+"/"+unitXName+".");
        if(maxSpeed<=0 || getAverageSpeed(values)<=0) txt_MaxAverage_Line.setHeight(0);
        else
            txt_MaxAverage_Line.setText("=> Lớn hơn "+df.format((maxSpeed/getAverageSpeed(values)-1)*100)+" % so với tốc độ tăng trưởng trung bình.");

        txt_MinSpeed_Line.setText(fieldName+" có tốc độ tăng trưởng chậm nhất là "+fields[getMinSpeedPosition(values)]+": "+df.format(minSpeed)+" "+unitYName+"/"+unitXName+".");
        if(minSpeed<=0 || getAverageSpeed(values)<=0) txt_MinAverage_Line.setHeight(0);
        else
            txt_MinAverage_Line.setText("=> Nhỏ hơn "+df.format((1-getAverageSpeed(values)/minSpeed)*100)+" % so với tốc độ tăng trưởng trung bình.");
    }
